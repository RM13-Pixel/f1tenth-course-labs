...
alpha_pub = rospy.Publisher('/alpha', Int64, queue_size = 1)
...
def getRanges(data,angle):
    ...
    dist = np.min(data.ranges[ind-10:ind+10])
...
def purepursuit_control_node(data):
    ...
    # ! 1.5 works
    lookahead_distance = 1.8  # Fine-tune this value according to your requirements
    ...
    alpha_pub.publish(int((alpha - lidarData.angle_min) / lidarData.angle_increment))
    ...
    # # else:
    # steering_angle = 3.3 * math.degrees(math.atan((2.0 * WHEELBASE_LEN * ((rotate_y)/distance_to_target))/(distance_to_target)))
    # # weighted_angle = 3.5 * steering_angle
    # if 1.4 < pose_x and pose_y < 0.921 and steering_angle < 0:
    # # if 1.014 < pose_x and 3.911 < pose_y:
    #     steering_angle = 7.5 * math.degrees(math.atan((2.0 * WHEELBASE_LEN * ((rotate_y)/distance_to_target))/(distance_to_target)))
    #     print("Big Steer!!")
    ...
    command.speed = (1-(abs(steering_angle))/135)* 65
    ...
    if lidarData != []:
        ...
        # ! different from getRange!
        lidarDist = getRange(lidarData, 0)
        # print("Dist: ", lidarDist)
        if lidarDist < distance_to_target and lidarDist > 0.05:
            # print("OBSTACLE! OBSTACLE! OBSTACLE!")
            if ftg_comm != []:
                ftg_comm.speed = command.speed
